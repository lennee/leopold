...
const listEvents = async (calendarId: string): Promise<calendar_v3.Schema$Event[]> =>
  (await _getAuthedCalendarClient(Scopes.FULL_CALENDAR)).events.list({
    calendarId,
...
const createEvent = async (calendarId: string, event: calendar_v3.Schema$Event): Promise<any> =>
  (await _getAuthedCalendarClient(Scopes.FULL_CALENDAR)).events.insert({
    calendarId,
    requestBody: event,
  })
